...
def get_model(DEBUG=False):
    ...
    # 畳み込み層
    conv_bn(model, 3, 128, ReLU(), True, True, kernel_regularizer=True)  # 0
    model.add(Dropout(0.25))
    for _ in range(1, 12):
        conv_bn(model, 3, 128, ReLU(), kernel_regularizer=True)
        model.add(Dropout(0.25))
    ...
    # 全結合層
    dense_bn(model, 256, ReLU(), kernel_regularizer=True)
